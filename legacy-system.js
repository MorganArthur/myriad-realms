...
const { cellStyle: legacyArtCellStyle } = globalThis.RealmArtAtlas;
...
function heroArtifactDetailHtml(hero) {
  ...
  return `<h3>携带神器</h3><div class="artifact-list">${carried.map(artifact => { const definition = artifactDefs[artifact.type]; return `<button class="artifact-chip has-art" data-legacy-entity="artifact:${artifact.id}" style="--artifact-color:${definition.color};${legacyArtCellStyle("artifacts", artifact.type)}">${definition.icon} ${definition.name} · ${Math.round(artifact.durability)}%</button>`; }).join("")}</div>`;
}
...
function inspectLegacyEntity(key) {
  ...
  if (kind === "artifact") { const artifact = artifacts.find(candidate => candidate.id === id); if (!artifact) return; const definition = artifactDefs[artifact.type], kingdom = getKingdom(artifactKingdomId(artifact)), history = artifact.history?.[0]; box.innerHTML = `<div class="legacy-atlas-art legacy-detail-art" style="${legacyArtCellStyle("artifacts", artifact.type)}" role="img" aria-label="${definition.name}原创像素插图"></div><h4 style="color:${definition.color}">${definition.icon} ${definition.name}</h4><div class="detail-row"><span>持有者 / 文明</span><b>${artifactHolderLabel(artifact)} · ${kingdom?.name || "无主"}</b></div><div class="detail-row"><span>状态 / 耐久</span><b>${artifact.status === "lost" ? "失落" : artifact.status === "damaged" ? "损毁" : "完好"} · ${Math.round(artifact.durability)}%</b></div><p class="muted">${definition.effect}${history ? `<br>纪元 ${history.year} · ${history.text}` : ""}</p>${artifact.status !== "lost" ? `<div class="legacy-actions"><button data-artifact-action="hero:${artifact.id}">交给英雄</button><button data-artifact-action="treasury:${artifact.id}">收归国库</button>${artifact.durability < 100 ? `<button data-artifact-action="repair:${artifact.id}">修复神器</button>` : ""}</div>` : ""}`; }
  if (kind === "wonder") { const wonder = wonders.find(candidate => candidate.id === id); if (!wonder) return; const definition = wonderDefs[wonder.type], kingdom = getKingdom(wonder.kingdomId), state = wonder.status === "complete" ? "已完成" : wonder.status === "damaged" ? "受损" : wonder.status === "ruined" ? "奇观遗迹" : "建造中"; box.innerHTML = `<div class="legacy-atlas-art legacy-detail-art" style="${legacyArtCellStyle("wonders", wonder.type)}" role="img" aria-label="${definition.name}原创像素插图"></div><h4>${definition.icon} ${definition.name}</h4><div class="detail-row"><span>所属文明</span><b>${kingdom?.name || "失落文明"}</b></div><div class="detail-row"><span>状态 / 奠基</span><b>${state} · 纪元 ${wonder.startedYear}</b></div><div class="need-list">${needMeter("工程进度", wonder.progress)}${needMeter("奇观耐久", wonder.hp / wonder.maxHp * 100)}</div><p class="muted">${definition.effect}${wonder.damageHistory?.[0] ? `<br>最近记录：纪元 ${wonder.damageHistory[0].year} · ${wonder.damageHistory[0].cause}` : ""}</p>${["damaged", "ruined"].includes(wonder.status) ? `<div class="legacy-actions"><button data-wonder-action="repair:${wonder.id}">${wonder.status === "ruined" ? "重建奇观" : "修复奇观"}</button></div>` : ""}`; }
}
...
function legacyDetailHtml(kingdom) {
  ...
  const artifactHtml = owned.map(artifact => { const definition = artifactDefs[artifact.type]; return `<button class="artifact-chip has-art" data-legacy-entity="artifact:${artifact.id}" style="--artifact-color:${definition.color};${legacyArtCellStyle("artifacts", artifact.type)}" title="${definition.effect}">${definition.icon} ${definition.name} · ${artifactHolderLabel(artifact)} · ${Math.round(artifact.durability)}%</button>`; }).join("") || `<span class="muted">尚未持有神器</span>`;
  const wonderHtml = wonder ? `<button class="wonder-project ${wonder.status} has-art" data-legacy-entity="wonder:${wonder.id}" style="--wonder-color:${wonderDefs[wonder.type].color};${legacyArtCellStyle("wonders", wonder.type)}"><b>${wonderDefs[wonder.type].icon} ${wonderDefs[wonder.type].name}</b><span>${wonder.status === "complete" ? "世界奇观已完成" : wonder.status === "damaged" ? `受损 · 耐久 ${Math.round(wonder.hp / wonder.maxHp * 100)}%` : `工程进度 ${Math.round(wonder.progress)}%`}</span><i><em style="width:${wonder.status === "building" ? wonder.progress : wonder.hp / wonder.maxHp * 100}%"></em></i></button>` : "";
...
function renderLegacyPanels() {
  ...
  if (legacyList) {
    ...
    const wonderItems = wonders.slice(0, 6).map(wonder => `<button class="legacy-site wonder ${wonder.status} has-art" data-legacy-entity="wonder:${wonder.id}" style="--legacy-color:${wonderDefs[wonder.type].color};${legacyArtCellStyle("wonders", wonder.type)}"><b>${wonderDefs[wonder.type].icon} ${wonderDefs[wonder.type].name}</b><span>${getKingdom(wonder.kingdomId)?.name || "失落文明"} · ${wonder.status === "complete" ? "已完成" : wonder.status === "damaged" ? `受损 ${Math.round(wonder.hp / wonder.maxHp * 100)}%` : wonder.status === "ruined" ? "已成遗迹" : `${Math.round(wonder.progress)}%`}</span><i><em style="width:${wonder.status === "building" ? wonder.progress : wonder.hp / wonder.maxHp * 100}%"></em></i></button>`).join("");
