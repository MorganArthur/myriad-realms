...
test("v22 存档无损保存遗迹、神器、奇观、危机与挑战状态", () => {
  const { debug } = createWorldRuntime();
...
test("v19 存档会补全神器持有、耐久与奇观耐久字段", () => {
  ...
  assert.equal(snapshot.legacy.wonders[0][5], 300); assert.equal(snapshot.legacy.wonders[0][6], 300); assert.equal(debug.saveData().version, 22);
});
